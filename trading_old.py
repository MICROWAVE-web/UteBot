...
class TradingBot:
    def __init__(self, url, userid, token):
        ...
        try:
            ...
            for i in self.serv_answ:
                if "Connection successful" in str(i):
                    # print('Connection successful')
                    continue
                try:
                    ute_data = json.loads(str(i[-1])[2:-1])
                    if "i_balance" in ute_data:
                        self.ute_data = ute_data
    ...
    def open_option(self, pair_name, up_dn, sum_option, type_account, time_h, time_m, time_s, percent_par):
        ...
        if 'Error' in self.serv_answ[-1][1].decode():
            ...
            print(exeptions_determniant(el))
        else:
            print(self.serv_answ[-1])
            try:
                ute_data = json.loads(str(self.serv_answ[-1])[2:-1])
                if "i_balance" in ute_data:
                    self.ute_data = ute_data
                    print(self.ute_data)
            except json.decoder.JSONDecodeError:
